...
def create_app() -> FastAPI:
    ...
    async def lifespan(_: FastAPI):
        ...
        register_service.start_auto_scheduler(stop_event)
        try:
            yield
        finally:
            stop_event.set()
            ...
            register_service.shutdown(timeout=5)
            backup_service.stop()
    app = FastAPI(title="chatgpt-2api", version=app_version, lifespan=lifespan)
