...
from services.browser_fingerprint import (
    ...
    chrome146_fingerprint,
    chrome146_headers,
    chrome146_signed_asset_headers,
    urls_share_browser_origin,
)
...
class _CurlResponseStream:
    """File-like view of a curl_cffi streaming response."""
...
class OpenAIBackendAPI:
    ...
    def _build_fp(self) -> Dict[str, str]:
        ...
        for key in ("oai-device-id", "oai-session-id"):
            value = str(account.get(key) or fp.get(key) or "").strip()
            if value:
                fp[key] = value
        fp.setdefault("oai-device-id", new_uuid())
        fp.setdefault("oai-session-id", new_uuid())
        fp.update(chrome146_fingerprint())
        return fp
    ...
    def _codex_responses_headers(self) -> Dict[str, str]:
        ...
        for key, value in dict(session_headers).items():
            if str(key).lower().startswith("oai-"):
