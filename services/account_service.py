...
class AccountService:
    ...
    def _is_account_selectable(
        ...
        *,
        ...
        if credential_availability.status == "unavailable":
            return False
        remote_check_result = str(account.get("last_remote_check_result") or "").strip().lower()
        if remote_check_result == "error" and not str(account.get("last_remote_checked_at") or "").strip():
            return False
        if remote_check_result != "pending":
            return True
        return bool(
            allow_image_pending
    ...
    def _dispatch_stats_for_token_locked(self, access_token: str) -> AccountDispatchStats:
        ...
        return AccountDispatchStats(
            inflight=inflight,
            generating_seconds=self._image_generating_seconds_locked(access_token, inflight),
    ...
    def _pick_ranked_image_token_locked(self, tokens: list[str]) -> str:
        ...
        ranked = rank_image_account_tokens(
            tokens,
            {token: self._dispatch_stats_for_token_locked(token) for token in tokens},
    ...
    def acquire_image_slot(self, access_token: str, deadline_monotonic: float | None = None) -> str:
        """Hold one image slot for an account that is already selected."""
        ...
        while True:
            with self._image_slot_condition:
                ...
                if remaining is not None and remaining <= 0:
                    raise TimeoutError("image account slot wait exceeded")
                resolved = self._resolve_access_token_locked(token)
                if resolved not in self._accounts:
                    raise ValueError("task account session is no longer available")
                max_concurrency = max(1, int(config.image_account_concurrency or 1))
                if int(self._image_inflight.get(resolved, 0)) < max_concurrency:
                    self._record_image_slot_start_locked(resolved)
    ...
    def get_account_by_access_token_fingerprint(self, fingerprint: str) -> dict | None:
        """Resolve an account from the stored SHA-256 of a current or rotated access token."""
        ...
        with self._lock:
            for token, account in self._accounts.items():
                fingerprints = self._normalize_access_token_fingerprints(
