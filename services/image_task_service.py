...
def _resume_access_token(task: Mapping[str, object]) -> str:
    ...
    account = account_service.get_account_by_access_token_fingerprint(
        _account_token_fingerprint(task.get("account_token_fingerprint")),
    )
    ...
    def _update_task(self, key: str, *, persist: bool = True, **updates: Any) -> None:
        with self._lock:
            ...
            for field, value in updates.items():
                if field == "account_token_fingerprint":
                    fingerprint = _account_token_fingerprint(value)
                    if fingerprint:
