...
POSTGRES_DB="${POSTGRES_DB_INPUT:-chatgpt_2api_app}"
POSTGRES_USER="${POSTGRES_USER_INPUT:-chatgpt_2api}"
...
normalize_database_choice() {
  ...
  value="${value//[[:space:]]/}"
  case "${value}" in
    postgres|postgres-local|local-postgres|local) printf 'postgres-local' ;;
    *) return 1 ;;
  esac
}
...
configure_database() {
  ...
  if [[ -n "${DATABASE_URL}" ]]; then
    ui_println "[$(text prefix_info)] 安装会创建本地 PostgreSQL 18，不使用已有的数据库地址"
  fi
  DATABASE_URL=""
  if [[ -n "${DATABASE_MODE}" ]]; then
    DATABASE_MODE="$(normalize_database_choice "${DATABASE_MODE}")" || {
      echo "[$(text prefix_error)] 数据库只支持本地 PostgreSQL 18" >&2
      exit 1
    }
  fi
  DATABASE_MODE="postgres-local"
  DATABASE_URL=""
  if [[ -n "${POSTGRES_DB_INPUT}" && "${POSTGRES_DB_INPUT}" != "chatgpt_2api_app" ]]; then
    echo "[$(text prefix_error)] 数据库名必须是 chatgpt_2api_app" >&2
    exit 1
  fi
  POSTGRES_DB="chatgpt_2api_app"
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --auth-key)
        ...
        ;;
      --database-url)
        echo "[$(text prefix_error)] 安装不需要填写 PostgreSQL 地址，会在本机创建数据库" >&2
        exit 1
        ;;
      --database)
        DATABASE_MODE="${2:-}"
...
POSTGRES_PASSWORD=${POSTGRES_PASSWORD}
TZ=Asia/Shanghai
COMPOSE_PROJECT_NAME=chatgpt-2api
EOF
...
run_docker() {
  ...
  local compose_args=(-p chatgpt-2api -f docker-compose.yml)
  if [[ "${DATABASE_MODE}" == "postgres-local" ]]; then
...
main() {
  ...
  MODE="$(normalize_mode_choice "${MODE}")" || { echo "[$(text prefix_error)] 只支持 Docker 与本地 PostgreSQL 18" >&2; exit 1; }
  if [[ "${MODE}" != "docker" ]]; then
    echo "[$(text prefix_error)] 只支持 Docker 与本地 PostgreSQL 18" >&2
    exit 1
  fi
  if [[ "${BRANCH}" != "main" ]]; then
    echo "[$(text prefix_error)] 安装分支固定为 main" >&2
    exit 1
  fi
  ...
  confirm_key="$(prompt_input "请再次输入管理员登录密钥" "")"
  if [[ "${AUTH_KEY}" != "${confirm_key}" ]]; then
    echo "[$(text prefix_error)] 两次输入的管理员登录密钥不一致" >&2
    exit 1
  fi
